...
class FlatOdfTests(unittest.TestCase):
    ...
    def test_flat_odf_preserves_chart_object_subpackage(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with zipfile.ZipFile(roundtrip) as archive:
                names = archive.namelist()
                object_content = [n for n in names if n.startswith("Object ") and n.endswith("/content.xml")]
                self.assertEqual(
                    len(object_content),
                    1,
            ...
            self.assertIn("chart:chart", chart_xml, "chart body must live in its own sub-package")
            self.assertNotIn("chart:chart", host_xml, "chart body must not leak into the host content.xml")
